...
class AuthMiddleware:
    """Hands every request the cookies the UI needs. It no longer refuses anyone.

    Until 2026-09-23 this was a gate: a browser needed a one-time link (``?k=``) from the
    tray or the launcher, and without the session cookie that link set, every request got
    a 401 "Authorize this browser" page. It kept other software and other Windows users on
    the same machine out of the transcripts — the Host check and CSRF only stop websites —
    but the cost was a sign-in on every browser profile, repeated after every restart,
    since the session secret is not kept across starts. That was judged not worth it for
    now, and the gate was removed; the risk and the candidate fixes are recorded in the
    ClickUp bug "Local UI has no access control" and in D57.

    What remains: the launcher's link endpoint (it still prints a ``?k=`` link, which the
    Windows launcher waits for), and the cookies. The CSRF double-submit needs the UI to
    hold the ``up_csrf`` cookie, which the gate used to set on redeeming a link; it is now
    set on any response to a request that does not already carry it. SameSite=Strict keeps
    a cross-site page from reading or sending it, so CSRF protection is unchanged.
    """
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        if scope.get("path") == LINK_PATH and scope.get("method") == "POST":
            # Still answered for the launcher's N key; the link now simply opens the app.
        ...
        if token:
            # Spent, stale or from another process: none of it matters any more.
            self.auth.redeem(token)
        if self.auth.valid_csrf(_cookies(scope).get(CSRF_COOKIE)):
            await self.app(scope, receive, send)
            return
